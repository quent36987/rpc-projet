...
class MilpSolverPriority(Solver):
    ...
    def solve(self, max_seconds=float('inf')) -> Result:
        ...
        # p[i],q[i],r[i],t[i] = product i dimensions (length, width, height), product priority (-1= no priority, 0 = important, 1= less important ... n = less less important)
        p, q, r, t = map(list, zip(*map(
            lambda product: [product.length, product.width, product.height, product.delivery_priority],
            self.inputs.product_list)))

        # Reverse the priority list number to have the most important with the highest number
        t_max = max(t)
        t = list(map(lambda x: 0 if x == -1 else t_max - x, t))
        ...
        model.objective = minimize(
            # Minimize the empty space inside the trucks
            xsum(L * W * H * n[j] for j in range(m)) # Minimize the number of trucks used
            - xsum(p[i] * q[i] * r[i] for i in range(N))

            # Minimize the distance from the end of the truck for important products
            + xsum((t[i]+1) * (L - (x[i] + (p[i] * lx[i] + q[i] * wx[i] + r[i] * hx[i]))) for i in range(N))
            + xsum((t[i]+1) * (W - (y[i] + (p[i] * ly[i] + q[i] * wy[i] + r[i] * hy[i]))) for i in range(N))
            + xsum((t[i]+1) * (H - (z[i] + (p[i] * lz[i] + q[i] * wz[i] + r[i] * hz[i]))) for i in range(N))
        )

        # todo: Gravity is ignored meaning that all cube wants to be on top of the truck, we should add a constraint to minimize the distance from the bottom of the truck
        # todo: Center of gravity is ignored, we should add a constraint to minimize the distance from the center of gravity
        ...
        for i in range(N):
            for k in range(N):
                if i < k:
                    model += x[i] + p[i] * lx[i] + q[i] * (lz[i] - wy[i] + hz[i]) + r[i] * (
                                1 - lx[i] - lz[i] + wy[i] - hz[i]) <= x[k] + (1 - a[i][k]) * M
                    model += x[k] + p[k] * lx[k] + q[k] * (lz[k] - wy[k] + hz[k]) + r[k] * (
                                1 - lx[k] - lz[k] + wy[k] - hz[k]) <= x[i] + (1 - b[i][k]) * M

                    model += y[i] + q[i] * wy[i] + p[i] * (1 - lx[i] - lz[i]) + r[i] * (lx[i] + lz[i] - wy[i]) <= y[
                        k] + (1 - c[i][k]) * M
                    model += y[k] + q[k] * wy[k] + p[k] * (1 - lx[k] - lz[k]) + r[k] * (lx[k] + lz[k] - wy[k]) <= y[
                        i] + (1 - d[i][k]) * M
        ...
        for i in range(N):
            for j in range(m):
                model += x[i] + p[i] * lx[i] + q[i] * (lz[i] - wy[i] + hz[i]) + r[i] * (
                            1 - lx[i] - lz[i] + wy[i] - hz[i]) <= L + (1 - s[i][j]) * M
                model += y[i] + q[i] * wy[i] + p[i] * (1 - lx[i] - lz[i]) + r[i] * (lx[i] + lz[i] - wy[i]) <= W + (
                            1 - s[i][j]) * M
